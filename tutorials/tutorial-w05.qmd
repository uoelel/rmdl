...
It is unusual to report the results of the varying effects (not that this is necessarily a good practice), but you should at least mention the varying effects you included when reporting the model specification. For example, if the model has `(A | participant)`:

> As multilevel (aka random) effects, I included by-participant varying intercepts and by-participant varying slopes for A.
